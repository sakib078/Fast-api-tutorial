...
const Auth = () => {
  ...
  const { isAuthenticated, login, signup, loading: authLoading } = useAuth();
  const navigate = useNavigate();

  useEffect(() => {
    // If we've finished checking and the user IS logged in, 
    // send them to the dashboard [web:77].
    if (!authLoading && isAuthenticated) {
      navigate('/');
    }
  }, [isAuthenticated, authLoading, navigate]);

  // Phase 1: Checking for existing cookie
  if (authLoading) {
    return (
  ...
  // Phase 2: If authenticated, return null while the redirect happens
  if (isAuthenticated) {
    return null;
  }
